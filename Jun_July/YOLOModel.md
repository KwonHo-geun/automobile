...
# YOLOv11n vs YOLOv12n 성능 비교 분석

YOLOv11n과 YOLOv12n 모델을 동일한 환경(입력 크기 288×640, 1306프레임 영상, Colab)에서 각각 테스트한 결과를 기반으로, 주요 속도 지표 및 프레임 처리 성능을 비교 분석합니다.
...
- **입력 사이즈:** (1, 3, 288, 640)
- **테스트 환경:** Google Colab
- **모델:** YOLOv11n, YOLOv12n (Nano 버전)

---

## 2. 속도 및 처리 성능 요약

| 모델        | Preprocess(ms) | Inference(ms) | Postprocess(ms) | 총 소요 시간(s) | 평균 FPS  |
|-------------|:--------------:|:-------------:|:---------------:|:--------------:|:--------:|
| YOLOv11n    |     3.2        |   105.2       |     1.3         |    176.57      |   7.40   |
| YOLOv12n    |     3.3        |   116.3       |     1.2         |    194.88      |   6.70   |
...
- **Inference 시간:**  
  YOLOv12n(116.3ms)이 YOLOv11n(105.2ms) 대비 약 10% 더 느림.
- **평균 FPS (초당 프레임 처리율):**  
  YOLOv11n이 7.40, YOLOv12n이 6.70으로 YOLOv11n이 약 10% 우세.
- **총 추론 소요 시간:**  
  YOLOv12n이 전체적으로 18초가량 더 오래 걸림(동일 프레임 기준).
- **Pre/Postprocess 속도:**  
  두 모델 모두 비슷한 수준(약 3ms, 1.2~1.3ms).

---

## 4. 결론 및 활용 시 참고점

- **YOLOv11n**은 동일 입력과 환경에서 더 빠른 FPS와 짧은 총 실행 시간으로 실시간성 또는 고속 처리가 중요한 프로젝트에 적합합니다.
- **YOLOv12n**은 최신 attention 구조 등으로 정확도가 일부 개선될 수 있으나, 처리 속도(FPS) 측면에서는 YOLOv11n 대비 소폭 느린 것으로 나타났습니다.
- 둘 다 nano 버전(경량형)이므로, 다양한 기기(엣지·모바일 등)에서 실시간 처리가 요구될 때 성능 참고 자료로 활용할 수 있습니다.
